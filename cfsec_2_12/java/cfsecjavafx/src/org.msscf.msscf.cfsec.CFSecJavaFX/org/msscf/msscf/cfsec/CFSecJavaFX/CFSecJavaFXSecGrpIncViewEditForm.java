// Description: Java 11 JavaFX View/Edit Form implementation for SecGrpInc.
...
/**
 *	CFSecJavaFXSecGrpIncViewEditForm JavaFX View/Edit Form implementation
 *	for SecGrpInc.
 */
public class CFSecJavaFXSecGrpIncViewEditForm
extends CFBorderPane
implements ICFSecJavaFXSecGrpIncPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit Security Group Include";
	...
	public CFSecJavaFXSecGrpIncViewEditForm( ICFFormManager formManager, ICFSecJavaFXSchema argSchema, ICFSecSecGrpIncObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecSecGrpIncObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getSecGrpIncFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFSecSecGrpIncEditObj editObj = (ICFSecSecGrpIncEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFSecJavaFXSecGrpIncPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFSecSecGrpIncObj argFocus = (ICFSecSecGrpIncObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecSecGrpIncObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsSecGrpInc() != null ) {
							if( null == getJavaFXFocusAsSecGrpInc().getEdit() ) {
			...
			buttonClose.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						cfFormManager.closeCurrentForm();
						if( formClosedCallback != null ) {
							focus = getJavaFXFocusAsSecGrpInc();
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFSecJavaFXSecGrpIncPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFSecJavaFXSecGrpIncPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}

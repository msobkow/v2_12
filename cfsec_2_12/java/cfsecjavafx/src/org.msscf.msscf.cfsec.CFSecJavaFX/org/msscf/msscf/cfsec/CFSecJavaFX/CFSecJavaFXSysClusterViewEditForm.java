// Description: Java 11 JavaFX View/Edit Form implementation for SysCluster.
...
/**
 *	CFSecJavaFXSysClusterViewEditForm JavaFX View/Edit Form implementation
 *	for SysCluster.
 */
public class CFSecJavaFXSysClusterViewEditForm
extends CFBorderPane
implements ICFSecJavaFXSysClusterPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit Cluster";
	...
	public CFSecJavaFXSysClusterViewEditForm( ICFFormManager formManager, ICFSecJavaFXSchema argSchema, ICFSecSysClusterObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecSysClusterObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getSysClusterFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFSecSysClusterEditObj editObj = (ICFSecSysClusterEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFSecJavaFXSysClusterPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFSecSysClusterObj argFocus = (ICFSecSysClusterObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecSysClusterObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsSysCluster() != null ) {
							if( null == getJavaFXFocusAsSysCluster().getEdit() ) {
			...
			buttonClose.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						cfFormManager.closeCurrentForm();
						if( formClosedCallback != null ) {
							focus = getJavaFXFocusAsSysCluster();
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFSecJavaFXSysClusterPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFSecJavaFXSysClusterPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}

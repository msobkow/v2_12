// Description: Java 11 JavaFX View/Edit Form implementation for ISOTZone.
...
/**
 *	CFSecJavaFXISOTZoneViewEditForm JavaFX View/Edit Form implementation
 *	for ISOTZone.
 */
public class CFSecJavaFXISOTZoneViewEditForm
extends CFBorderPane
implements ICFSecJavaFXISOTZonePaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit ISO Timezone";
	...
	public CFSecJavaFXISOTZoneViewEditForm( ICFFormManager formManager, ICFSecJavaFXSchema argSchema, ICFSecISOTZoneObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecISOTZoneObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getISOTZoneFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFSecISOTZoneEditObj editObj = (ICFSecISOTZoneEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFSecJavaFXISOTZonePaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFSecISOTZoneObj argFocus = (ICFSecISOTZoneObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecISOTZoneObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsISOTZone() != null ) {
							if( null == getJavaFXFocusAsISOTZone().getEdit() ) {
			...
			buttonClose.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						cfFormManager.closeCurrentForm();
						if( formClosedCallback != null ) {
							focus = getJavaFXFocusAsISOTZone();
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFSecJavaFXISOTZonePaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFSecJavaFXISOTZonePaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}

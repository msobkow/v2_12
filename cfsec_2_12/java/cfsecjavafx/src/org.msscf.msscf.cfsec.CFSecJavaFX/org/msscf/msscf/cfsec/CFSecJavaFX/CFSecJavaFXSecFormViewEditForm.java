// Description: Java 11 JavaFX View/Edit Form implementation for SecForm.
...
/**
 *	CFSecJavaFXSecFormViewEditForm JavaFX View/Edit Form implementation
 *	for SecForm.
 */
public class CFSecJavaFXSecFormViewEditForm
extends CFBorderPane
implements ICFSecJavaFXSecFormPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit Security Form";
	...
	public CFSecJavaFXSecFormViewEditForm( ICFFormManager formManager, ICFSecJavaFXSchema argSchema, ICFSecSecFormObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecSecFormObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getSecFormFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFSecSecFormEditObj editObj = (ICFSecSecFormEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFSecJavaFXSecFormPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFSecSecFormObj argFocus = (ICFSecSecFormObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecSecFormObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsSecForm() != null ) {
							if( null == getJavaFXFocusAsSecForm().getEdit() ) {
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFSecJavaFXSecFormPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFSecJavaFXSecFormPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}

// Description: Java 11 JavaFX View/Edit Form implementation for ISOCtryLang.
...
/**
 *	CFSecJavaFXISOCtryLangViewEditForm JavaFX View/Edit Form implementation
 *	for ISOCtryLang.
 */
public class CFSecJavaFXISOCtryLangViewEditForm
extends CFBorderPane
implements ICFSecJavaFXISOCtryLangPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit ISOCtryLang";
	...
	public CFSecJavaFXISOCtryLangViewEditForm( ICFFormManager formManager, ICFSecJavaFXSchema argSchema, ICFSecISOCtryLangObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecISOCtryLangObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getISOCtryLangFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFSecISOCtryLangEditObj editObj = (ICFSecISOCtryLangEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFSecJavaFXISOCtryLangPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFSecISOCtryLangObj argFocus = (ICFSecISOCtryLangObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecISOCtryLangObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsISOCtryLang() != null ) {
							if( null == getJavaFXFocusAsISOCtryLang().getEdit() ) {
			...
			buttonClose.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						cfFormManager.closeCurrentForm();
						if( formClosedCallback != null ) {
							focus = getJavaFXFocusAsISOCtryLang();
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFSecJavaFXISOCtryLangPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFSecJavaFXISOCtryLangPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}

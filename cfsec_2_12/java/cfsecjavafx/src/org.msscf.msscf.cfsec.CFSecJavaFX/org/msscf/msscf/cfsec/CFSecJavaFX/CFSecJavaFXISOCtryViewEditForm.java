// Description: Java 11 JavaFX View/Edit Form implementation for ISOCtry.
...
/**
 *	CFSecJavaFXISOCtryViewEditForm JavaFX View/Edit Form implementation
 *	for ISOCtry.
 */
public class CFSecJavaFXISOCtryViewEditForm
extends CFBorderPane
implements ICFSecJavaFXISOCtryPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit ISO Ctry";
	...
	public CFSecJavaFXISOCtryViewEditForm( ICFFormManager formManager, ICFSecJavaFXSchema argSchema, ICFSecISOCtryObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecISOCtryObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getISOCtryFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFSecISOCtryEditObj editObj = (ICFSecISOCtryEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFSecJavaFXISOCtryPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFSecISOCtryObj argFocus = (ICFSecISOCtryObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecISOCtryObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsISOCtry() != null ) {
							if( null == getJavaFXFocusAsISOCtry().getEdit() ) {
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFSecJavaFXISOCtryPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFSecJavaFXISOCtryPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}

// Description: Java 11 JavaFX View/Edit Form implementation for SecGroupForm.
...
/**
 *	CFSecJavaFXSecGroupFormViewEditForm JavaFX View/Edit Form implementation
 *	for SecGroupForm.
 */
public class CFSecJavaFXSecGroupFormViewEditForm
extends CFBorderPane
implements ICFSecJavaFXSecGroupFormPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit Security Group Form";
	...
	public CFSecJavaFXSecGroupFormViewEditForm( ICFFormManager formManager, ICFSecJavaFXSchema argSchema, ICFSecSecGroupFormObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecSecGroupFormObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getSecGroupFormFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFSecSecGroupFormEditObj editObj = (ICFSecSecGroupFormEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFSecJavaFXSecGroupFormPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFSecSecGroupFormObj argFocus = (ICFSecSecGroupFormObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecSecGroupFormObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsSecGroupForm() != null ) {
							if( null == getJavaFXFocusAsSecGroupForm().getEdit() ) {
			...
			buttonClose.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						cfFormManager.closeCurrentForm();
						if( formClosedCallback != null ) {
							focus = getJavaFXFocusAsSecGroupForm();
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFSecJavaFXSecGroupFormPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFSecJavaFXSecGroupFormPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}

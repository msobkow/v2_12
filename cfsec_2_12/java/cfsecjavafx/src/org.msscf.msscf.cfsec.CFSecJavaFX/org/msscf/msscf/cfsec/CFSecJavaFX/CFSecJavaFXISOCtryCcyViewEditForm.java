// Description: Java 11 JavaFX View/Edit Form implementation for ISOCtryCcy.
...
/**
 *	CFSecJavaFXISOCtryCcyViewEditForm JavaFX View/Edit Form implementation
 *	for ISOCtryCcy.
 */
public class CFSecJavaFXISOCtryCcyViewEditForm
extends CFBorderPane
implements ICFSecJavaFXISOCtryCcyPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit ISOCtryCcy";
	...
	public CFSecJavaFXISOCtryCcyViewEditForm( ICFFormManager formManager, ICFSecJavaFXSchema argSchema, ICFSecISOCtryCcyObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecISOCtryCcyObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getISOCtryCcyFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFSecISOCtryCcyEditObj editObj = (ICFSecISOCtryCcyEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFSecJavaFXISOCtryCcyPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFSecISOCtryCcyObj argFocus = (ICFSecISOCtryCcyObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecISOCtryCcyObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsISOCtryCcy() != null ) {
							if( null == getJavaFXFocusAsISOCtryCcy().getEdit() ) {
			...
			buttonClose.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						cfFormManager.closeCurrentForm();
						if( formClosedCallback != null ) {
							focus = getJavaFXFocusAsISOCtryCcy();
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFSecJavaFXISOCtryCcyPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFSecJavaFXISOCtryCcyPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}

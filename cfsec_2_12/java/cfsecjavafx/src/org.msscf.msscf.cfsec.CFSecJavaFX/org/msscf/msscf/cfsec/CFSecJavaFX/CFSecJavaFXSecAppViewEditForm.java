// Description: Java 11 JavaFX View/Edit Form implementation for SecApp.
...
/**
 *	CFSecJavaFXSecAppViewEditForm JavaFX View/Edit Form implementation
 *	for SecApp.
 */
public class CFSecJavaFXSecAppViewEditForm
extends CFBorderPane
implements ICFSecJavaFXSecAppPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit Security Application";
	...
	public CFSecJavaFXSecAppViewEditForm( ICFFormManager formManager, ICFSecJavaFXSchema argSchema, ICFSecSecAppObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecSecAppObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getSecAppFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFSecSecAppEditObj editObj = (ICFSecSecAppEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFSecJavaFXSecAppPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFSecSecAppObj argFocus = (ICFSecSecAppObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecSecAppObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsSecApp() != null ) {
							if( null == getJavaFXFocusAsSecApp().getEdit() ) {
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFSecJavaFXSecAppPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFSecJavaFXSecAppPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}

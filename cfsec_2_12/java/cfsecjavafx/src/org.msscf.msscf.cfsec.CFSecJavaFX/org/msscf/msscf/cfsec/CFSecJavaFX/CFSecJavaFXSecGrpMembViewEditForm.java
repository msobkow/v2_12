// Description: Java 11 JavaFX View/Edit Form implementation for SecGrpMemb.
...
/**
 *	CFSecJavaFXSecGrpMembViewEditForm JavaFX View/Edit Form implementation
 *	for SecGrpMemb.
 */
public class CFSecJavaFXSecGrpMembViewEditForm
extends CFBorderPane
implements ICFSecJavaFXSecGrpMembPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit Security Group Member";
	...
	public CFSecJavaFXSecGrpMembViewEditForm( ICFFormManager formManager, ICFSecJavaFXSchema argSchema, ICFSecSecGrpMembObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecSecGrpMembObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getSecGrpMembFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFSecSecGrpMembEditObj editObj = (ICFSecSecGrpMembEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFSecJavaFXSecGrpMembPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFSecSecGrpMembObj argFocus = (ICFSecSecGrpMembObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecSecGrpMembObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsSecGrpMemb() != null ) {
							if( null == getJavaFXFocusAsSecGrpMemb().getEdit() ) {
			...
			buttonClose.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						cfFormManager.closeCurrentForm();
						if( formClosedCallback != null ) {
							focus = getJavaFXFocusAsSecGrpMemb();
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFSecJavaFXSecGrpMembPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFSecJavaFXSecGrpMembPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}

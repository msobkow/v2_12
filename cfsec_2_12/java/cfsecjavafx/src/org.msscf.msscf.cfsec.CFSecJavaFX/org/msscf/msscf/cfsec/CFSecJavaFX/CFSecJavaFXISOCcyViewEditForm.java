// Description: Java 11 JavaFX View/Edit Form implementation for ISOCcy.
...
/**
 *	CFSecJavaFXISOCcyViewEditForm JavaFX View/Edit Form implementation
 *	for ISOCcy.
 */
public class CFSecJavaFXISOCcyViewEditForm
extends CFBorderPane
implements ICFSecJavaFXISOCcyPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit ISO Ccy";
	...
	public CFSecJavaFXISOCcyViewEditForm( ICFFormManager formManager, ICFSecJavaFXSchema argSchema, ICFSecISOCcyObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecISOCcyObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getISOCcyFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFSecISOCcyEditObj editObj = (ICFSecISOCcyEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFSecJavaFXISOCcyPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFSecISOCcyObj argFocus = (ICFSecISOCcyObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecISOCcyObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsISOCcy() != null ) {
							if( null == getJavaFXFocusAsISOCcy().getEdit() ) {
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFSecJavaFXISOCcyPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFSecJavaFXISOCcyPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}

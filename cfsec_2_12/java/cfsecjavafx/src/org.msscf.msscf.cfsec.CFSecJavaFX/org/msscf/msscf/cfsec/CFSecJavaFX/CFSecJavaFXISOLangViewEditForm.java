// Description: Java 11 JavaFX View/Edit Form implementation for ISOLang.
...
/**
 *	CFSecJavaFXISOLangViewEditForm JavaFX View/Edit Form implementation
 *	for ISOLang.
 */
public class CFSecJavaFXISOLangViewEditForm
extends CFBorderPane
implements ICFSecJavaFXISOLangPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit ISO Lang";
	...
	public CFSecJavaFXISOLangViewEditForm( ICFFormManager formManager, ICFSecJavaFXSchema argSchema, ICFSecISOLangObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecISOLangObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getISOLangFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFSecISOLangEditObj editObj = (ICFSecISOLangEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFSecJavaFXISOLangPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFSecISOLangObj argFocus = (ICFSecISOLangObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecISOLangObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsISOLang() != null ) {
							if( null == getJavaFXFocusAsISOLang().getEdit() ) {
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFSecJavaFXISOLangPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFSecJavaFXISOLangPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}

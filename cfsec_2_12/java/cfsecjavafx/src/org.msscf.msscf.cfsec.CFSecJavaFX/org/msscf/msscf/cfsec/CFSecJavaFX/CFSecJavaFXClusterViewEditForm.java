// Description: Java 11 JavaFX View/Edit Form implementation for Cluster.
...
/**
 *	CFSecJavaFXClusterViewEditForm JavaFX View/Edit Form implementation
 *	for Cluster.
 */
public class CFSecJavaFXClusterViewEditForm
	...
	ICFForm
{
	...
	public CFSecJavaFXClusterViewEditForm( ICFFormManager formManager, ICFSecJavaFXSchema argSchema, ICFSecClusterObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecClusterObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getClusterFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFSecClusterEditObj editObj = (ICFSecClusterEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFSecJavaFXClusterPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFSecClusterObj argFocus = (ICFSecClusterObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecClusterObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsCluster() != null ) {
							if( null == getJavaFXFocusAsCluster().getEdit() ) {
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFSecJavaFXClusterPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFSecJavaFXClusterPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}

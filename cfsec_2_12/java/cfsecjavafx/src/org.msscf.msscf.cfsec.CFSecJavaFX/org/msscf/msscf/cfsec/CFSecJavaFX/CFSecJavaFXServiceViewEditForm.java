// Description: Java 11 JavaFX View/Edit Form implementation for Service.
...
/**
 *	CFSecJavaFXServiceViewEditForm JavaFX View/Edit Form implementation
 *	for Service.
 */
public class CFSecJavaFXServiceViewEditForm
extends CFBorderPane
implements ICFSecJavaFXServicePaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit Service";
	...
	public CFSecJavaFXServiceViewEditForm( ICFFormManager formManager, ICFSecJavaFXSchema argSchema, ICFSecServiceObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecServiceObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getServiceFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFSecServiceEditObj editObj = (ICFSecServiceEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFSecJavaFXServicePaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFSecServiceObj argFocus = (ICFSecServiceObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecServiceObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsService() != null ) {
							if( null == getJavaFXFocusAsService().getEdit() ) {
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFSecJavaFXServicePaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFSecJavaFXServicePaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}

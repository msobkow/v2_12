// Description: Java 11 JavaFX View/Edit Form implementation for SecSession.
...
/**
 *	CFSecJavaFXSecSessionViewEditForm JavaFX View/Edit Form implementation
 *	for SecSession.
 */
public class CFSecJavaFXSecSessionViewEditForm
extends CFBorderPane
implements ICFSecJavaFXSecSessionPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit Security Session";
	...
	public CFSecJavaFXSecSessionViewEditForm( ICFFormManager formManager, ICFSecJavaFXSchema argSchema, ICFSecSecSessionObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecSecSessionObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getSecSessionFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFSecSecSessionEditObj editObj = (ICFSecSecSessionEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFSecJavaFXSecSessionPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFSecSecSessionObj argFocus = (ICFSecSecSessionObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFSecSecSessionObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsSecSession() != null ) {
							if( null == getJavaFXFocusAsSecSession().getEdit() ) {
			...
			buttonClose.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						cfFormManager.closeCurrentForm();
						if( formClosedCallback != null ) {
							focus = getJavaFXFocusAsSecSession();
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFSecJavaFXSecSessionPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFSecJavaFXSecSessionPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}

...
if [ "$Revision" == "" ]; then
	...
	cd $MSSCFHOME/cfsec_2_12/java
	if [ ! -d ../installer ]; then
		mkdir ../installer
	fi
	export Revision
	archiveFileName="../installer/cfsec_2_12_${Revision}-DualGPLv3-java-installer.zip"
	...
	apacheopt=""
	ls -1 ./bin/*.jar ./bin/lib/*.jar ${tomcatlibopt} gpl-3.0.txt LICENSE `find bin -name 'gpl-3.0.txt' -o -name 'LICENSE' -o -name '*.war' -o -name '*.xsd' -o -name '*.xml' -o -name '*.bash' -o -name '*.bat' -o -name '*.cmd' ` `find SampleServer -type f` | zip '-q9@' ${archiveFileName}

	...
	implements ICFGenKbTSecGroupTableObj
{
	...
	public void forgetTSecGroup( ICFGenKbTSecGroupObj Obj ) {
		forgetTSecGroup( Obj, false );
	}

	public void forgetTSecGroup( ICFGenKbTSecGroupObj Obj, boolean forgetSubObjects ) {
		ICFGenKbTSecGroupObj obj = Obj;
		CFGenKbTSecGroupPKey pkey = obj.getPKey();
		if( members.containsKey( pkey ) ) {
			ICFGenKbTSecGroupObj keepObj = members.get( pkey );
			...
			members.remove( pkey );
			if( forgetSubObjects ) {
				((ICFGenKbSchemaObj)schema).getTSecGrpIncTableObj().forgetTSecGrpIncByGroupIdx( keepObj.getRequiredTenantId(),
					keepObj.getRequiredTSecGroupId() );
				((ICFGenKbSchemaObj)schema).getTSecGrpMembTableObj().forgetTSecGrpMembByGroupIdx( keepObj.getRequiredTenantId(),
					keepObj.getRequiredTSecGroupId() );
				((ICFGenKbSchemaObj)schema).getTSecGrpIncTableObj().forgetTSecGrpIncByIncludeIdx( keepObj.getRequiredTenantId(),
					keepObj.getRequiredTSecGroupId() );
			}
		}
	}
		...
		int TSecGroupId )
	{
		...
		CFGenKbTSecGroupPKey key = ((ICFGenKbSchema)schema.getBackingStore()).getFactoryTSecGroup().newPKey();
		key.setRequiredTenantId( TenantId );
		key.setRequiredTSecGroupId( TSecGroupId );
		if( members.containsKey( key ) ) {
			ICFGenKbTSecGroupObj probed = members.get( key );
			if( probed != null ) {
				probed.forget( true );
			}
		}
	...
	public void forgetTSecGroupByTenantIdx( long TenantId )
	{
		...
		if( indexByTenantIdx.containsKey( key ) ) {
			Map<CFGenKbTSecGroupPKey, ICFGenKbTSecGroupObj > mapTenantIdx = indexByTenantIdx.get( key );
			if( mapTenantIdx != null ) {
				Iterator<ICFGenKbTSecGroupObj> iterDup = mapTenantIdx.values().iterator();
			...
			Iterator<ICFGenKbTSecGroupObj> memberIter = members.values().iterator();
		...
		Iterator<ICFGenKbTSecGroupObj> iter = toForget.iterator();
		while( iter.hasNext() ) {
			cur = iter.next();
			cur.forget( true );
		}
		...
		boolean IsVisible )
	{
		...
		if( indexByTenantVisIdx.containsKey( key ) ) {
			Map<CFGenKbTSecGroupPKey, ICFGenKbTSecGroupObj > mapTenantVisIdx = indexByTenantVisIdx.get( key );
			if( mapTenantVisIdx != null ) {
				Iterator<ICFGenKbTSecGroupObj> iterDup = mapTenantVisIdx.values().iterator();
				while( iterDup.hasNext() ) {
					cur = iterDup.next();
					toForget.add( cur );
				}

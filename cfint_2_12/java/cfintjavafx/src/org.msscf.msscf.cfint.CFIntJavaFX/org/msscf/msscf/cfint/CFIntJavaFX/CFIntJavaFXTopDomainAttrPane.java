// Description: Java 11 JavaFX Attribute Pane implementation for TopDomain.
...
/**
 *	CFIntJavaFXTopDomainAttrPane JavaFX Attribute Pane implementation
 *	for TopDomain.
 */
public class CFIntJavaFXTopDomainAttrPane
extends CFGridPane
implements ICFIntJavaFXTopDomainPaneCommon
{
		...
		extends CFInt64Editor
	{
		public IdEditor() {
			super();
			setMinValue( CFIntTopDomainBuff.ID_MIN_VALUE );
			setFieldName( "Id" );
		}
	}
	...
	public CFIntJavaFXTopDomainAttrPane( ICFFormManager formManager, ICFIntJavaFXSchema argSchema, ICFIntTopDomainObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFIntTopDomainObj getEffJavaFXFocus() {
		ICFIntTopDomainObj eff = (ICFIntTopDomainObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFIntTopDomainObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFIntTopDomainObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFIntTopDomainEditObj editObj;
		if( focus != null ) {
			editObj = (ICFIntTopDomainEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFIntTopDomainEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFIntTopDomainObj)editObj.create();

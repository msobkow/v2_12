// Description: Java 11 JavaFX Attribute Pane implementation for SubProject.
...
/**
 *	CFIntJavaFXSubProjectAttrPane JavaFX Attribute Pane implementation
 *	for SubProject.
 */
public class CFIntJavaFXSubProjectAttrPane
extends CFGridPane
implements ICFIntJavaFXSubProjectPaneCommon
{
		...
		extends CFInt64Editor
	{
		public IdEditor() {
			super();
			setMinValue( CFIntSubProjectBuff.ID_MIN_VALUE );
			setFieldName( "Id" );
		}
	}
	...
	public CFIntJavaFXSubProjectAttrPane( ICFFormManager formManager, ICFIntJavaFXSchema argSchema, ICFIntSubProjectObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFIntSubProjectObj getEffJavaFXFocus() {
		ICFIntSubProjectObj eff = (ICFIntSubProjectObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFIntSubProjectObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFIntSubProjectObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFIntSubProjectEditObj editObj;
		if( focus != null ) {
			editObj = (ICFIntSubProjectEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFIntSubProjectEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFIntSubProjectObj)editObj.create();

// Description: Java 11 JavaFX View/Edit Form implementation for TopProject.
...
/**
 *	CFIntJavaFXTopProjectViewEditForm JavaFX View/Edit Form implementation
 *	for TopProject.
 */
public class CFIntJavaFXTopProjectViewEditForm
extends CFBorderPane
implements ICFIntJavaFXTopProjectPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit TopProject";
	...
	public CFIntJavaFXTopProjectViewEditForm( ICFFormManager formManager, ICFIntJavaFXSchema argSchema, ICFIntTopProjectObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFIntTopProjectObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getTopProjectFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFIntTopProjectEditObj editObj = (ICFIntTopProjectEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFIntJavaFXTopProjectPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFIntTopProjectObj argFocus = (ICFIntTopProjectObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFIntTopProjectObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsTopProject() != null ) {
							if( null == getJavaFXFocusAsTopProject().getEdit() ) {
			...
			buttonClose.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						cfFormManager.closeCurrentForm();
						if( formClosedCallback != null ) {
							focus = getJavaFXFocusAsTopProject();
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFIntJavaFXTopProjectPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFIntJavaFXTopProjectPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}

// Description: Java 11 JavaFX Attribute Pane implementation for MajorVersion.
...
/**
 *	CFIntJavaFXMajorVersionAttrPane JavaFX Attribute Pane implementation
 *	for MajorVersion.
 */
public class CFIntJavaFXMajorVersionAttrPane
extends CFGridPane
implements ICFIntJavaFXMajorVersionPaneCommon
{
		...
		extends CFInt64Editor
	{
		public IdEditor() {
			super();
			setMinValue( CFIntMajorVersionBuff.ID_MIN_VALUE );
			setFieldName( "Id" );
		}
	}
	...
	public CFIntJavaFXMajorVersionAttrPane( ICFFormManager formManager, ICFIntJavaFXSchema argSchema, ICFIntMajorVersionObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFIntMajorVersionObj getEffJavaFXFocus() {
		ICFIntMajorVersionObj eff = (ICFIntMajorVersionObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFIntMajorVersionObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFIntMajorVersionObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFIntMajorVersionEditObj editObj;
		if( focus != null ) {
			editObj = (ICFIntMajorVersionEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFIntMajorVersionEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFIntMajorVersionObj)editObj.create();

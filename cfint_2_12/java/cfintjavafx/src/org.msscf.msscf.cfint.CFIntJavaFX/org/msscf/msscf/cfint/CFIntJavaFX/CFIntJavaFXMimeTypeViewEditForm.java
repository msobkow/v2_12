// Description: Java 11 JavaFX View/Edit Form implementation for MimeType.
...
/**
 *	CFIntJavaFXMimeTypeViewEditForm JavaFX View/Edit Form implementation
 *	for MimeType.
 */
public class CFIntJavaFXMimeTypeViewEditForm
extends CFBorderPane
implements ICFIntJavaFXMimeTypePaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit MIME Type";
	...
	public CFIntJavaFXMimeTypeViewEditForm( ICFFormManager formManager, ICFIntJavaFXSchema argSchema, ICFIntMimeTypeObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFIntMimeTypeObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getMimeTypeFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFIntMimeTypeEditObj editObj = (ICFIntMimeTypeEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFIntJavaFXMimeTypePaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFIntMimeTypeObj argFocus = (ICFIntMimeTypeObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFIntMimeTypeObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsMimeType() != null ) {
							if( null == getJavaFXFocusAsMimeType().getEdit() ) {
			...
			buttonClose.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						cfFormManager.closeCurrentForm();
						if( formClosedCallback != null ) {
							focus = getJavaFXFocusAsMimeType();
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFIntJavaFXMimeTypePaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFIntJavaFXMimeTypePaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}

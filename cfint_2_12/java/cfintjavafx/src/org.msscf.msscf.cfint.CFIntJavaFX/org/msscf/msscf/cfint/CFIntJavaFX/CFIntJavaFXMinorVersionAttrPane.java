// Description: Java 11 JavaFX Attribute Pane implementation for MinorVersion.
...
/**
 *	CFIntJavaFXMinorVersionAttrPane JavaFX Attribute Pane implementation
 *	for MinorVersion.
 */
public class CFIntJavaFXMinorVersionAttrPane
extends CFGridPane
implements ICFIntJavaFXMinorVersionPaneCommon
{
		...
		extends CFInt64Editor
	{
		public IdEditor() {
			super();
			setMinValue( CFIntMinorVersionBuff.ID_MIN_VALUE );
			setFieldName( "Id" );
		}
	}
	...
	public CFIntJavaFXMinorVersionAttrPane( ICFFormManager formManager, ICFIntJavaFXSchema argSchema, ICFIntMinorVersionObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsMinorVersion( argFocus );
	...
	public ICFIntMinorVersionObj getEffJavaFXFocus() {
		ICFIntMinorVersionObj eff = (ICFIntMinorVersionObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFIntMinorVersionObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFIntMinorVersionObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFIntMinorVersionEditObj editObj;
		if( focus != null ) {
			editObj = (ICFIntMinorVersionEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFIntMinorVersionEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFIntMinorVersionObj)editObj.create();

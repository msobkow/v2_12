// Description: Java 11 JavaFX View/Edit Form implementation for Tld.
...
/**
 *	CFIntJavaFXTldViewEditForm JavaFX View/Edit Form implementation
 *	for Tld.
 */
public class CFIntJavaFXTldViewEditForm
extends CFBorderPane
implements ICFIntJavaFXTldPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit Top Level Domain";
	...
	public CFIntJavaFXTldViewEditForm( ICFFormManager formManager, ICFIntJavaFXSchema argSchema, ICFIntTldObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFIntTldObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getTldFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFIntTldEditObj editObj = (ICFIntTldEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFIntJavaFXTldPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFIntTldObj argFocus = (ICFIntTldObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFIntTldObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsTld() != null ) {
							if( null == getJavaFXFocusAsTld().getEdit() ) {
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFIntJavaFXTldPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFIntJavaFXTldPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}

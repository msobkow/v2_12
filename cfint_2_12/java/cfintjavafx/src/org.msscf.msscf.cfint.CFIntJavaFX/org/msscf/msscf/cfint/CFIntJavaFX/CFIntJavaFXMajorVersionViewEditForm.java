// Description: Java 11 JavaFX View/Edit Form implementation for MajorVersion.
...
/**
 *	CFIntJavaFXMajorVersionViewEditForm JavaFX View/Edit Form implementation
 *	for MajorVersion.
 */
public class CFIntJavaFXMajorVersionViewEditForm
extends CFBorderPane
implements ICFIntJavaFXMajorVersionPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit MajorVersion";
	...
	public CFIntJavaFXMajorVersionViewEditForm( ICFFormManager formManager, ICFIntJavaFXSchema argSchema, ICFIntMajorVersionObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFIntMajorVersionObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getMajorVersionFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		ICFIntMajorVersionObj focus = getJavaFXFocusAsMajorVersion();
		if( focus != null ) {
			ICFIntMajorVersionEditObj editObj = (ICFIntMajorVersionEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFIntJavaFXMajorVersionPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFIntMajorVersionObj argFocus = (ICFIntMajorVersionObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFIntMajorVersionObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsMajorVersion() != null ) {
							if( null == getJavaFXFocusAsMajorVersion().getEdit() ) {
			...
			buttonClose.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						cfFormManager.closeCurrentForm();
						if( formClosedCallback != null ) {
							focus = getJavaFXFocusAsMajorVersion();
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFIntJavaFXMajorVersionPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFIntJavaFXMajorVersionPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}

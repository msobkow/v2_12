...
public class CFIntSecAppTableObj
	implements ICFIntSecAppTableObj
{
	...
	public CFIntSecAppTableObj( ICFSecSchemaObj argSchema ) {
		schema = argSchema;
		members = new HashMap<CFSecSecAppPKey, ICFSecSecAppObj>();
		allSecApp = null;
		indexByClusterIdx = null;
	...
	public ICFSecSecAppObj newInstance() {
		ICFSecSecAppObj inst = new CFIntSecAppObj( schema );
		return( inst );
	}
	...
	public ICFSecSecAppEditObj newEditInstance( ICFSecSecAppObj orig ) {
		ICFSecSecAppEditObj edit = new CFIntSecAppEditObj( orig );
		return( edit );
	}
		...
		if( members.containsKey( pkey ) && ( null != members.get( pkey ) ) ) {
			...
			if( indexByClusterIdx != null ) {
				CFSecSecAppByClusterIdxKey keyClusterIdx =
					((ICFIntSchema)schema.getBackingStore()).getFactorySecApp().newClusterIdxKey();
			...
			if( indexByUJEEMountIdx != null ) {
				CFSecSecAppByUJEEMountIdxKey keyUJEEMountIdx =
					((ICFIntSchema)schema.getBackingStore()).getFactorySecApp().newUJEEMountIdxKey();
		...
		int SecAppId )
	{
		...
		CFSecSecAppPKey key = ((ICFIntSchema)schema.getBackingStore()).getFactorySecApp().newPKey();
	...
	public void forgetSecAppByClusterIdx( long ClusterId )
	{
		...
		List<ICFSecSecAppObj> toForget = new LinkedList<ICFSecSecAppObj>();
		ICFSecSecAppObj cur = null;
		CFSecSecAppByClusterIdxKey key = ((ICFIntSchema)schema.getBackingStore()).getFactorySecApp().newClusterIdxKey();
		key.setRequiredClusterId( ClusterId );
		...
		String JEEMountName )
	{
		...
		List<ICFSecSecAppObj> toForget = new LinkedList<ICFSecSecAppObj>();
		ICFSecSecAppObj cur = null;
		CFSecSecAppByUJEEMountIdxKey key = ((ICFIntSchema)schema.getBackingStore()).getFactorySecApp().newUJEEMountIdxKey();
		key.setRequiredClusterId( ClusterId );
	...
	public ICFSecSecAppObj createSecApp( ICFSecSecAppObj Obj ) {
		...
		((ICFIntSchema)schema.getBackingStore()).getTableSecApp().createSecApp(
	...
	public ICFSecSecAppObj readSecApp( CFSecSecAppPKey pkey, boolean forceRead ) {
		...
		if( ( ! forceRead ) && members.containsKey( pkey ) ) {
			...
			CFSecSecAppBuff readBuff = ((ICFIntSchema)schema.getBackingStore()).getTableSecApp().readDerivedByIdIdx( schema.getAuthorization(),
				pkey.getRequiredClusterId(),
				pkey.getRequiredSecAppId() );
			if( readBuff != null ) {
				obj = schema.getSecAppTableObj().newInstance();
				obj.setPKey( ((ICFIntSchema)schema.getBackingStore()).getFactorySecApp().newPKey() );
	...
	public ICFSecSecAppObj lockSecApp( CFSecSecAppPKey pkey ) {
		...
		CFSecSecAppBuff lockBuff = ((ICFIntSchema)schema.getBackingStore()).getTableSecApp().lockDerived( schema.getAuthorization(), pkey );
		if( lockBuff != null ) {
				locked = schema.getSecAppTableObj().newInstance();
			locked.setPKey( ((ICFIntSchema)schema.getBackingStore()).getFactorySecApp().newPKey() );
	...
	public List<ICFSecSecAppObj> readAllSecApp( boolean forceRead ) {
		...
		if( ( allSecApp == null ) || forceRead ) {
			...
			allSecApp = map;
			CFSecSecAppBuff[] buffList = ((ICFIntSchema)schema.getBackingStore()).getTableSecApp().readAllDerived( schema.getAuthorization() );
		...
		Integer priorSecAppId )
	{
		...
		Map<CFSecSecAppPKey, ICFSecSecAppObj> map = new HashMap<CFSecSecAppPKey,ICFSecSecAppObj>();
		CFSecSecAppBuff[] buffList = ((ICFIntSchema)schema.getBackingStore()).getTableSecApp().pageAllBuff( schema.getAuthorization(),
			priorClusterId,
			priorSecAppId );
		CFSecSecAppBuff buff;
		ICFSecSecAppObj obj;
	...
	{
		CFSecSecAppPKey pkey = ((ICFIntSchema)schema.getBackingStore()).getFactorySecApp().newPKey();
		...
		boolean forceRead )
	{
		...
		if( ( ! forceRead ) && indexByClusterIdx.containsKey( key ) ) {
			...
			ICFSecSecAppObj obj;
			CFSecSecAppBuff[] buffList = ((ICFIntSchema)schema.getBackingStore()).getTableSecApp().readDerivedByClusterIdx( schema.getAuthorization(),
				ClusterId );
			CFSecSecAppBuff buff;
			for( int idx = 0; idx < buffList.length; idx ++ ) {
				buff = buffList[ idx ];
		...
		String JEEMountName, boolean forceRead )
	{
		...
		if( ( ! forceRead ) && indexByUJEEMountIdx.containsKey( key ) ) {
			...
			CFSecSecAppBuff buff = ((ICFIntSchema)schema.getBackingStore()).getTableSecApp().readDerivedByUJEEMountIdx( schema.getAuthorization(),
				ClusterId,
				JEEMountName );
			if( buff != null ) {
				obj = schema.getSecAppTableObj().newInstance();
		...
		Integer priorSecAppId )
	{
		...
		ICFSecSecAppObj obj;
		CFSecSecAppBuff[] buffList = ((ICFIntSchema)schema.getBackingStore()).getTableSecApp().pageBuffByClusterIdx( schema.getAuthorization(),
				ClusterId,
			priorClusterId,
			priorSecAppId );
		CFSecSecAppBuff buff;
	...
	public ICFSecSecAppObj updateSecApp( ICFSecSecAppObj Obj ) {
		ICFSecSecAppObj obj = Obj;
		((ICFIntSchema)schema.getBackingStore()).getTableSecApp().updateSecApp( schema.getAuthorization(),
			Obj.getSecAppBuff() );
		obj = (ICFSecSecAppObj)Obj.realise();
		obj.endEdit();
		return( obj );
	...
	public void deleteSecApp( ICFSecSecAppObj Obj ) {
		ICFSecSecAppObj obj = Obj;
		((ICFIntSchema)schema.getBackingStore()).getTableSecApp().deleteSecApp( schema.getAuthorization(),
			obj.getSecAppBuff() );
		obj.forget( true );
	}
	...
	public void deleteSecAppByClusterIdx( long ClusterId )
	{
		...
		if( indexByClusterIdx.containsKey( key ) ) {
			...
			((ICFIntSchema)schema.getBackingStore()).getTableSecApp().deleteSecAppByClusterIdx( schema.getAuthorization(),
				ClusterId );
		...
		String JEEMountName )
	{
		...
		if( indexByUJEEMountIdx.containsKey( key ) ) {
			obj = indexByUJEEMountIdx.get( key );
			((ICFIntSchema)schema.getBackingStore()).getTableSecApp().deleteSecAppByUJEEMountIdx( schema.getAuthorization(),
				ClusterId,
				JEEMountName );
			obj.forget( true );
		}

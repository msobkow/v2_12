 ...
 *	for Index.
 */
public class CFBamXmlLoaderPrimaryIndexHandler
	extends CFLibXmlCoreElementHandler
{
	public CFBamXmlLoaderPrimaryIndexHandler( CFBamXmlLoader saxLoader ) {
		super( saxLoader );
	}
	...
	throws SAXException
	{
		try {
			...
			assert qName.equals( "PrimaryIndex" );
			...
			// Make sure the table doesn't already have a PrimaryIndex

			if( null != refTable.getOptionalLookupPrimaryIndex() ) {
				throw new CFLibInvalidArgumentException( getClass(),
					S_ProcName,
					"Table \"" + refTable.getRequiredName() + "\" already specified a primary index" );
			}
			...
			ICFBamTableEditObj editTable = (ICFBamTableEditObj)refTable.beginEdit();
			editTable.setOptionalLookupPrimaryIndex( origIndex );
			editTable.update();
			editTable = null;
		}

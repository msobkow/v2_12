...
/*
 *	CFBamXmlLoaderLicenseParse XML SAX Element Handler implementation
 *	for License.
 */
public class CFBamXmlLoaderLicenseHandler
	extends CFLibXmlCoreElementHandler
{
	public CFBamXmlLoaderLicenseHandler( CFBamXmlLoader saxLoader ) {
		super( saxLoader );
	}
	...
	throws SAXException
	{
		try {
			...
			ICFBamTopDomainObj refDomain = null;
			...
			else if( scopeObj instanceof ICFBamTopDomainObj ) {
				refDomain = (ICFBamTopDomainObj) scopeObj;
				editBuff.setRequiredContainerTopDomain( refDomain );
			...
			ICFIntLicenseObj origLicense = schemaObj.getLicenseTableObj().readLicenseByUNameIdx( refTenant.getRequiredId(),
	...
	throws SAXException
	{
		...
		ICFBamLicenseObj license;
		if( curContext != null ) {
			license = (ICFBamLicenseObj)curContext.getNamedValue( "Object" );
		}
		else {
			throw new CFLibRuntimeException( getClass(),
				S_ProcName,
				"Context must be an existing License element" );
		}

		String text = curContext.getElementText();

		ICFBamLicenseEditObj editLicense = (ICFBamLicenseEditObj)license.beginEdit();
		editLicense.setOptionalEmbeddedText( text );
		editLicense.update();
		editLicense = null;
	}
}

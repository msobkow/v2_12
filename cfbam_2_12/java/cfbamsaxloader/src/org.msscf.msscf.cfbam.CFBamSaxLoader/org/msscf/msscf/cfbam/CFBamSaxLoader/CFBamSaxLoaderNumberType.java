...
// Description: Java 11 XML SAX Element Handler for NumberType
 ...
 *	for NumberType.
 */
public class CFBamSaxLoaderNumberType
	extends CFLibXmlCoreElementHandler
{
	public CFBamSaxLoaderNumberType( CFBamSaxLoader saxLoader ) {
		super( saxLoader );
	}
	...
	throws SAXException
	{
		try {
			...
			if( ( attrInitValue == null ) || ( attrInitValue.length() <= 0 ) ) {
				...
				natInitValue= CFLibBigDecimalUtil.parse( "CFBam.NumberDef.InitValue", 31, 5, attrInitValue );
			}
			...
			if( ( attrMinValue == null ) || ( attrMinValue.length() <= 0 ) ) {
				...
				natMinValue= CFLibBigDecimalUtil.parse( "CFBam.NumberDef.MinValue", 31, 5, attrMinValue );
			}
			...
			if( ( attrMaxValue == null ) || ( attrMaxValue.length() <= 0 ) ) {
				...
				natMaxValue= CFLibBigDecimalUtil.parse( "CFBam.NumberDef.MaxValue", 31, 5, attrMaxValue );
			}
			...
			ICFBamNumberTypeObj origNumberType = (ICFBamNumberTypeObj)schemaObj.getNumberTypeTableObj().readNumberTypeByUNameIdx( refTenant.getRequiredId(),
			...
			if( origNumberType == null ) {
				...
				switch( loaderBehaviour ) {
					...
					case Update:
						...
						editNumberType.setOptionalDefaultXmlValue( editBuff.getOptionalDefaultXmlValue() );
